...
export const CreateUserForm = ({ className }: { className?: string }) => {
  ...
  useEffect(() => {
    if (userTypes) setUserTypesOptions(convertToOptions({ data: userTypes, withAll: false }));
  }, [userTypes]);

  useEffect(() => {
    if (workerTypes) setWorkerTypesOptions(convertToOptions({ data: workerTypes, withAll: false }));
  }, [workerTypes]);
